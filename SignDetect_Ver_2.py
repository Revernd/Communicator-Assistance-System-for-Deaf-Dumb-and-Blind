...
model = load_model("SignNet_ISL.h5")
print("Model Loaded!!")
history = pd.read_csv("history_ISL.csv")
...
while True:
    ...
    cv2.putText(imgOrignal, "PROBABILITY: ", (20, 75), font, 0.5, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    if probabilityValue > 0.75:
        ...
        cv2.putText(imgOrignal, str(round(probabilityValue*100,2) )+"%", (180, 75), font, 0.5, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.imshow("Result", imgOrignal)
        if getClassName(classIndex)=='Nothing':
            word += " "
        else:
            word = word+str(getClassName(classIndex))
